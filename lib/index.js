...
RiakBackend.prototype.init = function(callback) {
  ...
  function _changelog() {
    if (!self.changelogBucket)
      return;
    var bucket = self.changelogBucket.name;
    return client.get(bucket, '');
    return client.put(self.changelogBucket.name, bucket.props, function(err) {
      if (++finished === 3)
        return callback(err);
    });
  }
  ...
  return this._operation(search, handlers, function() {
    return function setup(req, res, next) {
      req.riak = {
        ...
        client: self.client,
        indexes: ['dn', 'changetype', '_ctime', '_dtime', '_mtime'],
        key: req.dn.toString(),
