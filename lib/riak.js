...
Riak.prototype.listKeys = function(bucket, callback) {
  ...
  var keys = this._cacheGet(opts.path);
  if (keys)
  ...
  this._request(opts, function(err, obj, res) {
    ...
    self._cachePut(opts.path, obj.keys || []);
...
Riak.prototype.indexObject = function(indexes, object) {
  ...
  function _header(index, value) {
    ...
    headers['x-riak-index-' + index].push(' ' + qs.escape(value));
    return headers;
  }
...
Riak.prototype.fetchObjectsByIndex = function(bucket, index, value, callback) {
  ...
  function _escape(v, doubleEncode) {
    var esc = qs.escape(v);
    return doubleEncode ? qs.escape(esc) : esc;
  }

  function keysIterator(k) {
  ...
  var opts = {
    path: sprintf('/buckets/%s/index/%s', _escape(bucket), _escape(index, true))
  };
  value.forEach(function(v) {
    opts.path += '/' + _escape(v);
  });
  ...
  keys = this._cacheGet(opts.path);
  if (keys)
    return keys.forEach(keysIterator);
...
Riak.prototype._request = function(options, callback, write) {
  ...
  op.attempt(function(attempt) {
    ...
    var req = /^https.+/.test(self.url.protocol) ?
