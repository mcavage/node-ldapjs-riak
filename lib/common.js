...
function save(req, bucket, key, entry, meta, callback) {
  ...
  if (!entry._ctime) {
    entry._ctime = ISODateString();
    entry._created_from = req.logId;
    entry._created_by = req.connection.ldap.bindDN.toString();
  }
  entry._mtime = ISODateString();
  entry._modified_from = req.logId;
  entry._modified_by = req.connection.ldap.bindDN.toString();
