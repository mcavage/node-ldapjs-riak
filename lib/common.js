...
var PersistentSearchCache = require('./persistent_search_clients');

var PSC = new PersistentSearchCache();
...
function childExists(req, res, next) {
  ...
  var key = qs.escape(req.riak.key);
  var log = req.riak.log;

  log.debug('%s looking for children of %s', req.logId, key);
  return client.find(bucket, '_parent', key, true, function(err, entries) {
    if (err)
      return next(operationsError(err));

    if (entries && entries.length)
      return next(new ldap.NotAllowedOnNonLeafError(entries[0]));
...
function done(req, res, next) {
  ...
  if (req.persistentSearch) {
    ...
    PSC.addClient(req, res);
    res.connection.addListener('end', function() {
      // deregister the connection
      PSC.removeClient(req, res);
    });
  } else {
    res.end();
    return next();
...
function operationalAttributes(req, res, next) {
  ...
  if (!req.dn.equals(req.suffix))
    attributes._parent = [req.dn.parent().toString()];
...
function getEntryChangeNotificationControl(req, obj, callback) {
  ...
  if (req.persistentSearch.value.returnECs) {
    ...
    var ctrlOpts = {value: {}};
    ctrlOpts.value.changeType = getOperationType(attrs.changetype);
    // if it's a modDN request, fill in the previous DN
    if (ctrlOpts.value.changeType === 8) {
      ctrlOpts.value.previousDN = attrs.targetdn;
    }

    ctrlOpts.value.changeNumber = attrs.changenumber;
    return new EntryChangeNotificationControl(ctrlOpts);
  } else {
    return false;
  }
}
...
function updatePersistentSearchClients(req, res, next) {
  ...
  PSC.clientList.forEach(function(element, index, array) {
    // see if the change type of the PS request is the same as the current req
    if (checkChangeType(element.req, req.type)) {
      var changelog = res.changelog;
      var control = getEntryChangeNotificationControl(element.req, changelog);
      ...
      if (control) {
        // deep copy the object entry so we can tack on the control
        entry = JSON.parse(JSON.stringify(entry));
        entry.controls = [];
        entry.controls.push(control);
      }

      sendSearchRequest(element.req, element.res, entry, true, next);
    }
  });
...
function sendSearchRequest(req, res, obj, isPsc, callback) {
  var log = req.riak.log;
  var attrs = obj.attributes;
  ...
  if (isPsc) {
    // For persistent search requests, the dn used for matching is the obj.dn
    dn = obj.dn;
  } else {
    dn = req.dn;
  }
