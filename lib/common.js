...
function save(req, res, next) {
  ...
  if (req.riak.changelogBucket)
    entry.attributes.changenumber = res.changelog.attributes.changenumber;
  var key = req.riak.key;
  var log = req.riak.log;
...
function updatePersistentSearchClients(req, res, next) {
  ...
  PS.clientList.forEach(function(client) {
    ...
    if (PersistentSearch.checkChangeType(client.req, req.type)) {
      ...
      return sendSearchRequest(client.req, client.res, entry, control, next);
    }
  });
...
function sendSearchRequest(req, res, entry, controls, callback) {
  assert.ok(req);
  assert.ok(res);
  assert.ok(entry);
  ...
  if (send) {
    if (controls) {
      ...
      if (controls.isArray) {
        ...
        entry.controls = [];
        entry.controls.push(controls);
      }
    }

    log.debug('%s sending: %j', req.logId, entry);
    res.send(entry, req.hidden);
  }
