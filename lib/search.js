...
function _send(req, res, obj, callback) {
  ...
  if (typeof(req.searchCallback) !== 'function')
    return common.sendSearchRequest(req, res, obj,
      req.persistentSearch, callback);
  ...
  return req.searchCallback(req, obj, function(err, _obj) {
    ...
    return common.sendSearchRequest(req, res, _obj || obj,
      req.persistentSearch, callback);
...
function subtreeSearch(req, res, next) {
  ...
  return client.listKeys(bucket, function(err, keys) {
    ...
    keys.forEach(function(k) {
      var dn = parseDN(k);
      // add persistent search here because we're always returning changelogs
      // and the baseObject is actually the dn we're searching for, and not
      // the changelog, so changelog matchin of objectclass=* would fail here
